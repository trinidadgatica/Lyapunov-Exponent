...
class BubbleModelBase:
    ...
    def enthalpy(self, r: np.ndarray, j: np.ndarray, time: np.ndarray, n: float, A: float, B: float) -> np.ndarray:
        ...
        self.eq_1_history.append(eq_1)
        self.eq_2_history.append(eq_2)
    ...
    def dimensional_enthalpy(self, r: np.ndarray, j: np.ndarray, time: np.ndarray, n: float, A: float, B: float) -> np.ndarray:
        ...
        radius = r
        velocity = j
        pressure_infty = self.acoustic_pressure * np.sin(self.angular_frequency * time) + self.atmospheric_pressure

        eq_1 = (self.atmospheric_pressure + 2 * self.surface_tension / self.initial_radius) \
               * (self.initial_radius / radius) ** (3 * self.adiabatic_index)

        eq_2 = eq_1 - 2 * self.surface_tension / radius - 4 * self.viscosity * velocity / radius
        eq_3 = n * ((A) ** (1 / n)) / ((n - 1) * self.density)
        eq_4 = eq_3 * ((eq_2 + B) ** (1 - (1 / n)) - (pressure_infty + B ) ** (1 - (1 / n)))
